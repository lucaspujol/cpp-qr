 ...
 *
 * Each row lists the coordinate values used to generate all center positions
 * by taking every combination of two values from the row. Positions that
 * overlap finder patterns (top-left, top-right, bottom-left corners) are
 * skipped at placement time.
 *
 * -1 = no more coordinates for that version.
 *
 * Source: https://www.thonky.com/qr-code-tutorial/alignment-pattern-locations
 */
constexpr int16_t ALIGNMENT_PATTERN_POSITIONS[40][7] = {
    { -1, -1, -1, -1, -1, -1, -1 }, // Version 1  (none)
